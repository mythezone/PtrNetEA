...
class TSPDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        solution = torch.from_numpy(self.data['Solutions'][idx]).long() if self.solve else -1
...
if __name__ == '__main__':
    ...
    for p in [100,500,1000]:
        if p<=50:
            ...
            solve=False 
        x=TSPDataset(2000,p,solve=solve)
        with open('Datasets/TSPTest_%d.pkl'%p,'wb') as f:
            pickle.dump(x,f)
    ...
    for p in [100,500,1000]:
        ...
        with open('Datasets/TSPTrain_%d.pkl'%p,'wb') as f:
            pickle.dump(x,f)
